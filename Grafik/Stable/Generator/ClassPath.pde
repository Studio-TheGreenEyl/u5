class Path {
  ...
  void initText() {
    textString = "";
    indexOffset = (int)map(indexOffsetFloat, 0f, 1f, 0, blocks.size());

    for (int i = 0; i<blocks.size(); i++) textString += blocks.get( (i+indexOffset)%blocks.size() ).getString() + " ";
  }
    ...
    if (blocks.size() > 0) {
      ...
      for (int i = 0; i<coords.length; i++) {
        ...
        if (l > 0) {
          ...
          buffer[currentBuffer].textFont(myFont.get(floor(size*9)));
    ...
    if (blocks.size() > 0) {
      ...
      for (int i = 0; i<cutoff-1; i++) {
        ...
        if (l > 0) {
          ...
          float size = map(i, 0, coords.length, fontSizes[0], fontSizes[1]);
          if (state == EXPORT) {
  ...
  void setFontSizes(float a, float b) {
    /*rX[0] = (int)map(a, 0f, 1f, 0, 360);
     rX[1] = (int)map(b, 0f, 1f, 0, 360);
     */
    fontSizeArray[0] = a;
  ...
  void setCutoff(float f) {
    cutoff = (int)map(f, 0f, 1f, 0, coords.length);
    cutoffFloat = f;
  }
  ...
  void setIndexOffset(float f) {
    indexOffsetFloat = f;
  }

  float getIndexOffset() {
    return indexOffsetFloat;
  }
